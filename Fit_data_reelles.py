...
def categorie_data_labels(data):
    ...
    CSP = [0, 'Sans activite', 'Chomeur', 'Etudiant', 'Retraite', 'Agriculteur', 'Artisan', 'Commercant', 'Employes',
    ...
    cat_jur = [0, 'Personne Physique', 'Association', 'Cooperative', 'EXPLOITANT AGRICOLE', 'ARTISAN-COMMERCANT',
    ...
    data["CRED_Null_Group_interv_tiers"] = data["CRED_Null_Group_interv_tiers"].replace(Int_tiers, [i for i in range(
        len(Int_tiers))])
    ...
    Bien_finance = [0, 'Logement', 'Matériel PRO', 'Energie', 'Terrain', 'Bâtiment PRO', 'Amenagement foncier',
                    'Part sociale', 'Social', 'Animaux', 'Plant', 'Véhicule', 'Stock Agricole', 'Electronique menager',
                    'Divers']
    data["CRED_Null_Group_bien_fin_tiers"] = data["CRED_Null_Group_bien_fin_tiers"].replace(Bien_finance, [i for i in
                                                                                                           range(
                                                                                                               len(Bien_finance))])
    Bien_finance_hab = [0, 'Logement', 'Matériel PRO', 'Energie', 'Terrain', 'Part sociale', 'Divers']
    data["CRED_Null_Group_bien_fin_Hab"] = data["CRED_Null_Group_bien_fin_Hab"].replace(Bien_finance_hab, [i for i in
                                                                                                           range(
                                                                                                               len(Bien_finance_hab))])
    Bien_finance_conso = [0, 'Logement', 'Matériel PRO', 'Energie', 'Social', 'Véhicule', 'Electronique menager',
                          'Divers']
    data["CRED_Null_Group_bien_fin_Conso"] = data["CRED_Null_Group_bien_fin_Conso"].replace(Bien_finance_conso,
                                                                                            [i for i in range(
                                                                                                len(Bien_finance_conso))])

    Destination = [0, 'Logement', 'Foncier Logement', 'Matériel PRO', 'Foncier Agricole', 'Tresorerie', 'Bâtiment PRO',
                   'Animaux', 'Plants', 'Part sociale', 'Calamité', 'Rien', 'Divers']
    data["CRED_Null_Group_Dest_fin_tiers"] = data["CRED_Null_Group_Dest_fin_tiers"].replace(Destination, [i for i in
                                                                                                          range(
                                                                                                              len(Destination))])
    Destination_hab = [0, 'Logement', 'Foncier Logement', 'Tresorerie', 'Divers']
    data["CRED_Null_Group_Dest_fin_Hab"] = data["CRED_Null_Group_Dest_fin_Hab"].replace(Destination_hab, [i for i in
                                                                                                          range(
                                                                                                              len(Destination_hab))])
    Destination_conso = [0, 'Logement', 'Matériel PRO', 'Tresorerie', 'Rien', 'Divers']
    data["CRED_Null_Group_Dest_fin_Conso"] = data["CRED_Null_Group_Dest_fin_Conso"].replace(Destination_conso,
                                                                                            [i for i in range(
                                                                                                len(Destination_conso))])

    Cat_pret = [0, 'Immobilier', 'Professionnel', 'Prêt personnel', 'Ressources Propres', 'Agricole', 'Calamite',
                'Autre']
    data["CRED_Null_Regroup_CATEG_PRET"] = data["CRED_Null_Regroup_CATEG_PRET"].replace(Cat_pret, [i for i in range(
        len(Cat_pret))])
    data["CRED_Null_Regroup_CATEG_HAB"] = data["CRED_Null_Regroup_CATEG_HAB"].replace(Cat_pret,
                                                                                      [i for i in range(len(Cat_pret))])
    data["CRED_Null_Regroup_CATEG_CONSO"] = data["CRED_Null_Regroup_CATEG_CONSO"].replace(Cat_pret, [i for i in range(
        len(Cat_pret))])
    data["CRED_Null_Regroup_CATEG_REV"] = data["CRED_Null_Regroup_CATEG_REV"].replace(Cat_pret,
                                                                                      [i for i in range(len(Cat_pret))])

    Cat_pro = [0, 'Immobilier', 'Professionnel', 'Prêt personnel', 'Ressources Propres', 'Agricole', 'Calamite',
